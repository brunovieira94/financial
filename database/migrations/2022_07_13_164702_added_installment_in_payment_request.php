        ...
        foreach (PaymentRequest::withoutGlobalScopes()->get() as $paymentRequest) {
            if (!PaymentRequestHasInstallments::where('payment_request_id', $paymentRequest['id'])->exists()) {
                ...
                PaymentRequestHasInstallments::create([
                    ...
                    'due_date' => $paymentRequest['pay_date'] ?? $paymentRequest['emission_date'],
                    'status' => 0,
                    'extension_date' => $paymentRequest['pay_date'] ?? $paymentRequest['emission_date'],
                    'competence_date' => $paymentRequest['emission_date'],
                    'fess' => 0,
                    'discount' => 0,
