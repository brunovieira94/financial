        ...
        if (array_key_exists('installment_purchase_order', $paymentRequestInfo)) {
            ...
            foreach ($paymentRequestInfo['installment_purchase_order'] as $instalmentPurchaseOrder) {
                ...
                if ($request->route('id') != null) {
                    if(PaymentRequestHasPurchaseOrderInstallments::where('payment_request_id', $request->route('id'))->where('purchase_order_has_installments_id')->exists()){
                        $paymentRequestHasPurchaseOrderInstallments = PaymentRequestHasPurchaseOrderInstallments::where('payment_request_id', $request->route('id'))->where('purchase_order_has_installments_id')->first();
                ...
                if ($installmentPurchase->amount_paid > $installmentPurchase->portion_amount) {
